...
test.describe('Gallery Block - Interactive', () => {
	test.beforeEach(async ({ page }) => {
		...
		const response = await page.goto('/gallery-test/', { timeout: 5000 }).catch(() => null);
	...
	test('navigation buttons exist when gallery is present', async ({ page }) => {
		const galleryWrapper = page.locator('[data-gallery-wrapper]');

		if ((await galleryWrapper.count()) > 0) {
			const prevButton = galleryWrapper.locator('[data-gallery-previous]');
	...
	test('gallery scrolls on navigation click', async ({ page }) => {
		...
		const scrollLeft = await gallery.evaluate((el: HTMLElement) => el.scrollLeft);
