...
test.describe( 'Logo Grid Block', () => {
	...
	test( 'should apply grayscale filter when enabled', async ( { page } ) => {
		const logoImage = page
			.locator( '.wp-block-theme-oh-my-brand-logo-grid__image' )
			.first();

		// Check computed style includes grayscale
		const filter = await logoImage.evaluate(
	...
	test( 'should show color on hover when enabled', async ( { page } ) => {
		...
		if ( hasColorOnHover ) {
			const logoItem = page
				.locator( '.wp-block-theme-oh-my-brand-logo-grid__item' )
				.first();
			const logoImage = logoItem.locator(
				'.wp-block-theme-oh-my-brand-logo-grid__image'
			);

			// Get initial filter
			const initialFilter = await logoImage.evaluate(
				( el ) => getComputedStyle( el ).filter
			);
			...
			// Get hovered filter
			const hoveredFilter = await logoImage.evaluate(
				( el ) => getComputedStyle( el ).filter
			);
	...
	test( 'should respect CSS custom properties for columns', async ( {
		page,
	} ) => {
		const logoGrid = page.locator(
			'.wp-block-theme-oh-my-brand-logo-grid'
		...
		const columns = await logoGrid.evaluate( ( el ) =>
			getComputedStyle( el ).getPropertyValue( '--logo-grid-columns' )
		);
...
test.describe( 'Logo Grid Block Accessibility', () => {
	...
	test( 'should have semantic list structure', async ( { page } ) => {
		...
		const tagName = await list.evaluate( ( el ) =>
		...
		const firstItemTag = await items
			.first()
...
test.describe( 'Logo Grid Block Responsive', () => {
	test( 'should display correct columns on mobile', async ( { page } ) => {
		...
		const gridColumns = await logoList.evaluate(
