...
test.describe('Breadcrumbs Block', () => {
	...
	test('has proper navigation landmark', async ({ page }) => {
		...
		const block = page.locator('.wp-block-theme-oh-my-brand-breadcrumbs');
		if ((await block.count()) > 0) {
			await expect(block.first()).toHaveAttribute('aria-label', 'Breadcrumb');
		}
	});
	...
	test('contains ordered list structure', async ({ page }) => {
		...
		const block = page.locator('.wp-block-theme-oh-my-brand-breadcrumbs');
		if ((await block.count()) > 0) {
			const list = block.first().locator('ol.wp-block-theme-oh-my-brand-breadcrumbs__list');
	...
	test('current page has aria-current attribute', async ({ page }) => {
		...
		const block = page.locator('.wp-block-theme-oh-my-brand-breadcrumbs');
		if ((await block.count()) > 0) {
			const currentItem = block.first().locator('[aria-current="page"]');
	...
	test('links are keyboard navigable', async ({ page }) => {
		...
		const block = page.locator('.wp-block-theme-oh-my-brand-breadcrumbs');
		if ((await block.count()) > 0) {
			const links = block.first().locator('.wp-block-theme-oh-my-brand-breadcrumbs__link');
...
test.describe('Breadcrumbs Block - Accessibility', () => {
	test('separators are hidden from screen readers', async ({ page }) => {
		...
		const block = page.locator('.wp-block-theme-oh-my-brand-breadcrumbs');
		if ((await block.count()) > 0) {
			const separators = block.first().locator('.wp-block-theme-oh-my-brand-breadcrumbs__separator');
...
test.describe('Breadcrumbs Block - Mobile Truncation', () => {
	test('ellipsis button expands hidden items', async ({ page }) => {
		...
		const block = page.locator('.wp-block-theme-oh-my-brand-breadcrumbs--truncatable');
		if ((await block.count()) > 0) {
			const ellipsisButton = block.first().locator('.wp-block-theme-oh-my-brand-breadcrumbs__ellipsis');
			if ((await ellipsisButton.count()) > 0) {
				await ellipsisButton.click();
				await expect(block.first()).toHaveClass(/wp-block-theme-oh-my-brand-breadcrumbs--expanded/);
			}
		}
	});
});
