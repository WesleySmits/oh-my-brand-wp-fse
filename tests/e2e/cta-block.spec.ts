...
test.describe('CTA Block - Interactive', () => {
	test.beforeEach(async ({ page }) => {
		...
		const response = await page.goto('/cta-test/', { timeout: 5000 }).catch(() => null);
	...
	test('CTA heading is visible', async ({ page }) => {
		const ctaHeading = page.locator('.wp-block-theme-oh-my-brand-cta__heading');
	...
	test('CTA buttons are clickable', async ({ page }) => {
		const primaryButton = page.locator('.wp-block-theme-oh-my-brand-cta__button--primary').first();
	...
	test('secondary button exists when configured', async ({ page }) => {
		const secondaryButton = page.locator('.wp-block-theme-oh-my-brand-cta__button--secondary').first();
	...
	test('buttons have accessible text', async ({ page }) => {
		const buttons = page.locator('.wp-block-theme-oh-my-brand-cta__button');
...
test.describe('CTA Block - Accessibility', () => {
	...
	test('CTA heading has proper heading level', async ({ page }) => {
		const ctaBlock = page.locator('.wp-block-theme-oh-my-brand-cta').first();
		...
		const h2 = ctaBlock.locator('h2.wp-block-theme-oh-my-brand-cta__heading');
		const h3 = ctaBlock.locator('h3.wp-block-theme-oh-my-brand-cta__heading');
		const h4 = ctaBlock.locator('h4.wp-block-theme-oh-my-brand-cta__heading');

		const hasHeading = (await h2.count()) > 0 || (await h3.count()) > 0 || (await h4.count()) > 0;
		expect(hasHeading).toBe(true);
	});

	test('buttons are keyboard focusable', async ({ page }) => {
		const button = page.locator('.wp-block-theme-oh-my-brand-cta__button').first();
	...
	test('links with new tab have rel attribute', async ({ page }) => {
		const buttonsWithTarget = page.locator('.wp-block-theme-oh-my-brand-cta__button[target="_blank"]');
