...
test.describe('Accessibility', () => {
	test('gallery block should have no violations', async ({ page }) => {
		...
		const results = await new AxeBuilder({ page }).include('.wp-block-theme-oh-my-brand-gallery').analyze();
	...
	test('FAQ block should have no violations', async ({ page }) => {
		...
		const results = await new AxeBuilder({ page }).include('.wp-block-acf-faq').analyze();
	...
	test('hero block should have no violations', async ({ page }) => {
		...
		const results = await new AxeBuilder({ page }).include('.wp-block-theme-oh-my-brand-hero').analyze();

		expect(results.violations).toEqual([]);
	});

	test('entire page should have no critical violations', async ({ page }) => {
		await page.goto('/');
