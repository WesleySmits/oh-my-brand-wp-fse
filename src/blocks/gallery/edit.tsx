...
import { Button, Placeholder, PanelBody, RangeControl } from '@wordpress/components';
...
export default function Edit({ attributes, setAttributes }: BlockEditProps<GalleryAttributes>): JSX.Element {
	...
	const onSelectImages = (selectedImages: MediaItem[]): void => {
		const newImages: GalleryImage[] = selectedImages.map((image) => ({
			id: image.id,
			url: image.url,
			fullUrl: image.sizes?.full?.url || image.sizes?.large?.url || image.url,
			alt: image.alt || '',
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Gallery Settings', 'theme-oh-my-brand')} initialOpen={true}>
					<RangeControl
						label={__('Visible Images', 'theme-oh-my-brand')}
						help={__('Number of images visible at once.', 'theme-oh-my-brand')}
						value={visibleImages}
							...
							render={({ open }: { open: () => void }) => (
								...
								>
									<Button variant="primary" onClick={open}>
										{__('Select Images', 'theme-oh-my-brand')}
									</Button>
								</Placeholder>
							)}
						/>
						...
						<div className="wp-block-theme-oh-my-brand-gallery__inner">
							...
							>
								{images.map((image: GalleryImage, index: number) => (
									<div key={image.id || index} className="wp-block-theme-oh-my-brand-gallery__item">
										<img src={image.url} alt={image.alt} />
										<Button
											className="wp-block-theme-oh-my-brand-gallery__remove"
											icon="no-alt"
											label={__('Remove image', 'theme-oh-my-brand')}
						...
						<div className="wp-block-theme-oh-my-brand-gallery__toolbar">
							<MediaUploadCheck>
								<MediaUpload
									...
									multiple
									gallery
									value={images.map((img: GalleryImage) => img.id)}
									render={({ open }: { open: () => void }) => (
										<Button variant="secondary" onClick={open}>
											{__('Edit Gallery', 'theme-oh-my-brand')}
										</Button>
									)}
								/>
							</MediaUploadCheck>
