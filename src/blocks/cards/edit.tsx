...
export default function Edit( {
	attributes,
	setAttributes,
}: BlockEditProps< CardsAttributes > ): JSX.Element {
	const {
		heading,
		headingLevel,
		description,
	...
	return (
		<>
			<InspectorControls>
				{ /* Section Settings */ }
				<PanelBody
					title={ __( 'Section Settings', 'oh-my-brand' ) }
					initialOpen={ true }
				>
				...
				{ /* Grid Settings */ }
				<PanelBody
					title={ __( 'Grid Settings', 'oh-my-brand' ) }
					initialOpen={ true }
				>
					<RangeControl
						label={ __( 'Maximum Columns', 'oh-my-brand' ) }
						help={ __(
							'Grid will automatically show fewer columns on smaller screens.',
							'oh-my-brand'
						) }
						value={ columns }
						onChange={ ( value ) =>
				...
				{ /* Card Settings */ }
				<PanelBody
					title={ __( 'Card Settings', 'oh-my-brand' ) }
					initialOpen={ false }
				>
				...
				{ /* Image Settings */ }
				<PanelBody
					title={ __( 'Image Settings', 'oh-my-brand' ) }
					initialOpen={ false }
				>
					<ToggleControl
			...
			<section { ...blockProps }>
				...
				{ cards.length === 0 ? (
					<Placeholder
						...
						instructions={ __(
							'Add cards to display content in a grid layout.',
							'oh-my-brand'
						) }
					>
					...
					<ul className="wp-block-theme-oh-my-brand-cards__list">
						{ cards.map( ( card, index ) => (
							<li
								key={ card.id }
								className="wp-block-theme-oh-my-brand-cards__item"
							>
