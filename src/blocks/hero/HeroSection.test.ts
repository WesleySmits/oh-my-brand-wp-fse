...
describe('HeroSection', () => {
	...
	describe('initialization', () => {
		...
		it('should render internal structure', () => {
			...
			const background = element.shadowRoot?.querySelector('.hero__background');
			const overlay = element.shadowRoot?.querySelector('.hero__overlay');
			const content = element.shadowRoot?.querySelector('.hero__content');
		...
		it('should have background slot', () => {
			const slot = element.shadowRoot?.querySelector('slot[name="background"]');
	...
	describe('observed attributes', () => {
		...
		it('should observe overlay-opacity attribute', () => {
			expect(HeroSection.observedAttributes).toContain('overlay-opacity');
		});

		it('should observe content-align attribute', () => {
			expect(HeroSection.observedAttributes).toContain('content-align');
		});

		it('should observe vertical-align attribute', () => {
			expect(HeroSection.observedAttributes).toContain('vertical-align');
		});
	});
	...
	describe('attribute changes', () => {
		it('should update min-height CSS custom property', () => {
			...
			expect(element.style.getPropertyValue('--hero-min-height')).toBe('80vh');
		});

		it('should update overlay-opacity CSS custom property', () => {
			element.setAttribute('overlay-opacity', '0.7');

			expect(element.style.getPropertyValue('--hero-overlay-opacity')).toBe('0.7');
		});
	});
	...
	describe('slotted content', () => {
		it('should accept slotted background image', () => {
			...
			const slottedImg = element.querySelector('img[slot="background"]');
	...
	describe('cleanup', () => {
		it('should disconnect intersection observer on disconnectedCallback', () => {
			...
			element.intersectionObserver = mockObserver as unknown as IntersectionObserver;
