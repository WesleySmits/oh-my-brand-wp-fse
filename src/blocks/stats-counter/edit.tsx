...
function generateStatId(): string {
	return `stat-${ Date.now() }-${ Math.random()
		.toString( 36 )
		.substring( 2, 11 ) }`;
}
...
export default function Edit( {
	attributes,
	setAttributes,
}: BlockEditProps< StatsCounterAttributes > ): JSX.Element {
	const {
		heading,
		headingLevel,
		description,
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={ __( 'Section Settings', 'theme-oh-my-brand' ) }
				>
					<SelectControl
				...
				>
					...
					<SelectControl
						...
						onChange={ ( value ) =>
							setAttributes( { statStyle: value as StatStyle } )
						}
					/>
					<SelectControl
						...
						onChange={ ( value ) =>
							setAttributes( {
								contentAlignment: value as ContentAlignment,
							} )
						}
					/>
				</PanelBody>
					...
					<>
						<div className="wp-block-theme-oh-my-brand-stats-counter__list">
							{ stats.map( ( stat, index ) => (
								<div
									key={ stat.id }
									className="wp-block-theme-oh-my-brand-stats-counter__item"
								>
									{ showIcons && (
										<TextControl
