...
class OmbStatsCounter extends HTMLElement {
	...
	#animateCounters(): void {
		const counters = this.querySelectorAll<HTMLElement>('[data-counter]');
	...
	#updateCounterDisplay(counter: HTMLElement, value: number): void {
		...
		const formattedValue = this.#formatNumber(value, decimals, isCurrency, currencyCode, locale);
	...
	#formatNumber(value: number, decimals: number, isCurrency: boolean, currencyCode: string, locale: string): string {
		if (isCurrency) {
