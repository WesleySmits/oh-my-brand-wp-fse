...
import { TextControl, Placeholder, PanelBody, ToggleControl, Spinner, Notice } from '@wordpress/components';
...
function getVideoId(input: string): string | null {
	...
	const iframeMatch = input.match(/src="https?:\/\/www.youtube.com\/embed\/([a-zA-Z0-9_-]{11})/);
	if (iframeMatch) {
...
export default function Edit({ attributes, setAttributes }: BlockEditProps<YouTubeAttributes>): JSX.Element {
	...
	useEffect(() => {
		...
		const normalizedUrl = url.includes('youtube.com') || url.includes('youtu.be') ? url : null;
		...
		apiFetch<OEmbedResponse>({
			path: `/oembed/1.0/proxy?url=${encodeURIComponent(normalizedUrl)}`
		})
			.then((response) => {
				if (response?.title) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Video Settings', 'theme-oh-my-brand')} initialOpen={true}>
					<TextControl
						label={__('YouTube URL', 'theme-oh-my-brand')}
						help={__('Paste a YouTube video URL or embed code.', 'theme-oh-my-brand')}
						value={url}
					...
					{hasVideo && (
						<div className="wp-block-theme-oh-my-brand-youtube__title-field">
							<TextControl
								label={__('Video Title', 'theme-oh-my-brand')}
								help={
									isFetchingTitle
										? __('Fetching title…', 'theme-oh-my-brand')
										: __(
					...
					<ToggleControl
						...
						checked={lazyLoad}
						onChange={(value: boolean) => setAttributes({ lazyLoad: value })}
					/>
				</PanelBody>
			</InspectorControls>
			...
			<div {...blockProps}>
				{!hasVideo ? (
					<Placeholder
						...
						instructions={__('Paste a YouTube video URL in the sidebar settings.', 'theme-oh-my-brand')}
					>
						<TextControl
							label={__('YouTube URL', 'theme-oh-my-brand')}
							value={url}
					...
					<div className="wp-block-theme-oh-my-brand-youtube__preview">
						{videoTitle && (
							<p className="wp-block-theme-oh-my-brand-youtube__preview-title">{videoTitle}</p>
						)}
						<div className="wp-block-theme-oh-my-brand-youtube__preview-wrapper">
							<img
								src={getThumbnailUrl(videoId)}
								alt={videoTitle || __('YouTube video thumbnail', 'theme-oh-my-brand')}
