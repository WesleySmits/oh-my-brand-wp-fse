 ...
 */
export default function Edit({ attributes, setAttributes }: BlockEditProps<BannerAttributes>): JSX.Element {
	const {
		heading,
		headingLevel,
		content,
	...
	return (
		<>
			<InspectorControls>
				{/* Layout Settings */}
				<PanelBody title={__('Layout Settings', 'theme-oh-my-brand')} initialOpen={true}>
					<SelectControl
						...
						onChange={(value: string) => setAttributes({ imagePosition: value })}
					/>
					...
					<SelectControl
						...
						onChange={(value: string) => setAttributes({ imageSize: value })}
					/>
					...
					<SelectControl
						...
						onChange={(value: string) => setAttributes({ imageFit: value })}
					/>
					...
					<SelectControl
						label={__('Mobile Stack Order', 'theme-oh-my-brand')}
						value={mobileStack}
						options={[
							{
							...
							{
								label: __('Content First', 'theme-oh-my-brand'),
								value: 'content-first'
							}
						]}
						onChange={(value: string) => setAttributes({ mobileStack: value })}
					/>
				</PanelBody>

				{/* Content Settings */}
				<PanelBody title={__('Content Settings', 'theme-oh-my-brand')} initialOpen={false}>
					<SelectControl
				...
				{/* Image Settings */}
				<PanelBody title={__('Image', 'theme-oh-my-brand')} initialOpen={false}>
					<MediaUploadCheck>
						<MediaUpload
								...
								<>
									{image?.url ? (
										...
										<Button variant="primary" onClick={open}>
											{__('Select Image', 'theme-oh-my-brand')}
										</Button>
									)}
								</>
							)}
				...
				<ButtonSettingsPanel
					...
					button={button || { text: '', url: '', openInNewTab: false }}
					onChange={(newButton: BlockButton) => setAttributes({ button: newButton })}
				/>
			</InspectorControls>
						...
						<MediaUploadCheck>
							<MediaUpload
								...
								render={({ open }) => (
									<Placeholder
										icon={bannerIcon}
										label={__('Banner Image', 'theme-oh-my-brand')}
										instructions={__(
