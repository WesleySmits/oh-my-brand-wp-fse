...
export default function Edit( {
	attributes,
	setAttributes,
}: BlockEditProps< LogoGridAttributes > ): JSX.Element {
	const {
		heading,
		headingLevel,
		source,
	...
	useEffect( () => {
		...
		apiFetch< Record< string, PostType > >( { path: '/wp/v2/types' } )
			.then( ( types: Record< string, PostType > ) => {
				...
				const publicTypes = (
					Object.values( types ) as PostType[]
				 ).filter(
					( type: PostType ) =>
						type.rest_base && type.slug !== 'attachment'
				);
			...
			.catch( () => {
				setError(
					__( 'Failed to load post types.', 'theme-oh-my-brand' )
				);
				setIsLoadingPostTypes( false );
			} );
	}, [ source ] );
	...
	useEffect( () => {
		...
		apiFetch< MetaField[] >( {
			path: `/theme-oh-my-brand/v1/meta-fields/${ postType }`,
		} )
	...
	useEffect( () => {
		...
		apiFetch< LogoImage[] >( {
			path: `/theme-oh-my-brand/v1/logo-grid-preview?${ queryParams }`,
		} )
			...
			.catch( () => {
				setError(
					__(
						'Failed to load logos from selected post type.',
						'theme-oh-my-brand'
					)
				);
				setIsLoadingLogos( false );
	...
	const sourceOptions: Array< { label: string; value: SourceType } > = [
		{
			label: __( 'Manual Selection', 'theme-oh-my-brand' ),
			value: 'manual',
		},
		{
			label: __( 'Dynamic (from Post Type)', 'theme-oh-my-brand' ),
			value: 'dynamic',
		},
	];
	...
	const imageSourceOptions = [
		{
			label: __( 'Featured Image', 'theme-oh-my-brand' ),
	...
	const linkSourceOptions = [
		{
			label: __( 'Post Permalink', 'theme-oh-my-brand' ),
			value: 'permalink',
		},
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={ __( 'Data Source', 'theme-oh-my-brand' ) }
					initialOpen={ true }
				>
					<SelectControl< SourceType >
						...
						help={
							source === 'dynamic'
								? __(
										'Logos will be pulled dynamically from a post type.',
										'theme-oh-my-brand'
								  )
								: __(
										'Manually select logos from the media library.',
										'theme-oh-my-brand'
								  )
						}
					/>
							...
							{ postType && (
								<>
									<SelectControl
										...
										help={ __(
											'Where to get the logo image from.',
											'theme-oh-my-brand'
										) }
									/>
				...
				<PanelBody
					title={ __( 'Grid Settings', 'theme-oh-my-brand' ) }
					initialOpen={ false }
				>
					{ enableMarquee && (
						<Notice status="info" isDismissible={ false }>
							{ __(
								'Grid columns are disabled when marquee is enabled.',
								'theme-oh-my-brand'
							) }
						</Notice>
					) }
					...
					<RangeControl
						...
						value={ logoMaxHeight }
						onChange={ ( value ) =>
							setAttributes( { logoMaxHeight: value ?? 80 } )
						}
						min={ 40 }
						max={ 200 }
						step={ 10 }
				...
				<PanelBody
					title={ __( 'Animation Settings', 'theme-oh-my-brand' ) }
					initialOpen={ false }
				>
					<ToggleControl
						label={ __( 'Enable Marquee', 'theme-oh-my-brand' ) }
						help={ __(
							'Display logos in a continuous horizontal scroll.',
							'theme-oh-my-brand'
						) }
					...
					{ enableMarquee && displayLogos.length >= 4 && (
						<>
							...
							<ToggleControl
								...
								help={ __(
									'Pause the animation when users hover over logos.',
									'theme-oh-my-brand'
								) }
				...
				>
					...
					{ grayscale && (
						<ToggleControl
							...
							help={ __(
								'Show original colors when hovering over logos.',
								'theme-oh-my-brand'
							) }
							checked={ colorOnHover }
							onChange={ ( value ) =>
								setAttributes( { colorOnHover: value } )
							}
						/>
					) }
				</PanelBody>
				...
				{ source === 'manual' && hasLogos && (
					<PanelBody
						title={ __( 'Logo Links', 'theme-oh-my-brand' ) }
						initialOpen={ false }
					>
			...
			<section { ...blockProps }>
				...
				{ source === 'dynamic' && ! postType && (
					<Placeholder
						...
						instructions={ __(
							'Select a post type in the sidebar to display logos.',
							'theme-oh-my-brand'
						) }
					/>
				) }
						...
						<MediaUpload
							...
							render={ ( { open }: { open: () => void } ) => (
								<Placeholder
									...
									instructions={ __(
										'Select logos to display in the grid.',
										'theme-oh-my-brand'
									) }
								>
					...
					<>
						<ul className="wp-block-theme-oh-my-brand-logo-grid__list">
							{ displayLogos.map( ( image, index ) => (
								<li
									key={ image.id || index }
									className="wp-block-theme-oh-my-brand-logo-grid__item"
								>
									<img
										src={ image.url }
										alt={ image.alt }
						...
						{ source === 'dynamic' && (
							<p className="wp-block-theme-oh-my-brand-logo-grid__dynamic-notice">
								{ __(
									'Showing preview. Logos are loaded dynamically from:',
									'theme-oh-my-brand'
								) }{ ' ' }
