 ...
 * @param attributes
 */
function getCSSCustomProperties(attributes: LogoGridAttributes): Record<string, string> {
	return {
...
describe('Logo Grid Block', () => {
	...
	describe('Class Names', () => {
		it('should include base class', () => {
			...
			expect(classes).toContain('wp-block-theme-oh-my-brand-logo-grid');
		});
		...
		it('should include color-on-hover modifier when both grayscale and colorOnHover are true', () => {
			...
			expect(classes).toContain('wp-block-theme-oh-my-brand-logo-grid--color-on-hover');
		});
		...
		it('should not include color-on-hover modifier when grayscale is false', () => {
			...
			expect(classes).not.toContain('wp-block-theme-oh-my-brand-logo-grid--color-on-hover');
		});
	...
	describe('Column Range', () => {
		it('should support 2-6 desktop columns', () => {
			[2, 3, 4, 5, 6].forEach((columns) => {
				...
				expect(props['--logo-grid-columns']).toBe(String(columns));
			});
		});
		...
		it('should support 1-3 mobile columns', () => {
			[1, 2, 3].forEach((columnsMobile) => {
				...
				expect(props['--logo-grid-columns-mobile']).toBe(String(columnsMobile));
			});
		});
	});
});
