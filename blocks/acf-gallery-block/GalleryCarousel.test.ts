...
describe( 'GalleryCarousel', () => {
	...
	const createGalleryDOM = (
		options: { visibleImages?: number; itemCount?: number } = {}
	): HTMLElement => {
	...
	describe( 'initialization', () => {
		...
		it( 'should throw error if wrapper is null', () => {
			expect(
				() => new GalleryCarousel( null as unknown as HTMLElement )
			).toThrow( 'Required wrapper element not found' );
		} );

		it( 'should throw error if wrapper is already initialized', () => {
			new GalleryCarousel( wrapper );
			expect( () => new GalleryCarousel( wrapper ) ).toThrow(
		...
		it( 'should throw error if required elements are missing', () => {
			...
			expect( () => new GalleryCarousel( emptyWrapper ) ).toThrow(
	...
	describe( 'visible images configuration', () => {
		it( 'should read visible images from data attribute', () => {
			...
			expect( gallery.style.getPropertyValue( '--visible-images' ) ).toBe(
				'5'
			);
		} );
		...
		it( 'should handle different visible image values', () => {
			[ 1, 2, 4, 6 ].forEach( ( count ) => {
				...
				expect(
					gallery.style.getPropertyValue( '--visible-images' )
				).toBe( count.toString() );
			} );
		} );
	} );
